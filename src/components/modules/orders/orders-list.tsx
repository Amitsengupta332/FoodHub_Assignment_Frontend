...
import { Button } from "@/components/ui/button";
import { cancelMyOrder } from "@/actions/order.server";
import ReviewModal from "@/components/modules/reviews/review-modal";
...
export default function OrdersList({ orders }: any) {
  ...
  const [reviewMealId, setReviewMealId] = useState<string>("");
  ...
  const openReview = (mealId: string) => {
    setReviewMealId(mealId);
    setReviewOpen(true);
  };
        ...
        <div className="space-y-3">
          {orders.map((o: any) => (
            <div key={o.id} className="border p-4 rounded space-y-2">
              ...
              <p className="text-sm text-gray-600">Address: {o.address}</p>
              ...
              <div className="flex gap-2 pt-2">
                ...
                {/* Review (only DELIVERED) */}
                {o.status === "DELIVERED" ? (
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => openReview(o.items?.[0]?.mealId)}
                    disabled={!o.items?.[0]?.mealId}>
                    Leave Review
                  </Button>
                ) : null}
              </div>
